...
const calculateScoreRank = (score: number): string => {
  ...
  return 'D';
      ...
      // 强制刷新得分显示
      game.debug('强制刷新得分显示（遗留方法）', {}, 'force-refresh-score-legacy');
      // 如果得分为0但有净资产，尝试基于净资产计算一个默认得分
      if (this.getFinalScore === 0 && this.gameStats.finalAssets > 0) {
        game.debug('检测到得分为0但有净资产，尝试计算默认得分（遗留方法）', { finalAssets: this.gameStats.finalAssets }, 'calculate-default-score-legacy');
        // 直接基于净资产计算得分
