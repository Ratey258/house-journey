 ...
 * 价格历史记录接口
 */
interface PriceHistoryMap {
  [productId: number]: PriceHistoryData;
}
 ...
 * 测试不同商品类别的价格波动
 */
function testPriceFluctuations(): boolean {
  ...
  // 测试不同周数的价格变化
  const testWeeks: number[] = [1, 5, 10, 20, 30, 40, 52];
 ...
 * 测试地点价格修正
 */
function testLocationModifiers(): void {
  console.log('\n===== 地点价格修正测试 =====');
 ...
 * 测试市场修正因子
 */
function testMarketModifiers(): void {
  console.log('\n===== 市场修正因子测试 =====');
  ...
  // 初始价格数据
  const initialPriceData: PriceHistoryData = {
    price: testProduct.basePrice,
    trend: PriceTrend.STABLE,
    history: [testProduct.basePrice]
  };
 ...
 * 运行所有测试
 */
export function runPriceSystemTests(): boolean {
...
// 如果直接执行此文件，运行测试
if (typeof window !== 'undefined' && (window as any).runTest) {
  runPriceSystemTests();
}
