/**
 * 房屋实体 - 重构自 src/core/models/house.ts
 * 保持原项目的完整逻辑和数据结构
 */
 ...
 * 封装房屋的属性和行为
 */
export class House implements BaseEntity, Timestamps {
  ...
  constructor({
    ...
    this.image = image || this.getHouseImage(id);
  }
   ...
   * 获取房屋图片路径
   */
  private getHouseImage(houseId: HouseId): string {
    try {
